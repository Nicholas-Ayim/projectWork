...
import { selectAllContacts,newFilledForm,filledForm } from "../features/chatSlice"
 ...
 const handleEditForm = (e)=>{
  ...
  return(
    ...
    <div className="dashboard-title">
        <h4>{managerSelected.hostelManaged.toUpperCase()} HOSTEL MANAGER'S DASHBOARD</h4>
    </div>
    ...
    <div className="manager-sidebar">
      <div className="manager-sidebar-nest">
        ...
        <img src={managerSelected?.picture} className="manager-pic" alt="no-image"/>
        <p className="hostelManaged-name"><small>hostel: </small>{managerSelected.hostelManaged.substr(0,10)}</p>
        <p className="hostelManaged-name"><small>call: </small>{managerSelected.contact.substr(0,10)}</p>
        <p className="manager-online">{managerSelected.status}</p>
        <small>notification</small>
        {checkForm && <small onClick={handleEditForm}>Edit Form</small>}
        <small onClick={handleEditForm}>{formExist ?  "Edit Form" : ""}</small>
